...
use correlation_clustering::{greedy_cc_iterations, boem_cc_iterations, cluster_total_distance};
...
fn normalize_correlations(
    correlations: &mut Array4::<f64>,
    cutoff: f64
) {
    // Normalizes such that correlation at cutoff corresponds to 0
    // and that 0 < p < 1
    correlations.mapv_inplace(|a| if a >= cutoff {
...
pub fn update_file(
    ...
    let (lower_bound, higher_bound) = bounds;
    match open_gray_image(&Path::new(&format!("examples/cluster_data/{}_{}.tiff", lower_bound, higher_bound))) {
        Ok(data) => {
            ...
            if current < last {
                println!("File for bounds {:?} to {:?} replaced with better solution", lower_bound, higher_bound);
                return write_file(clusters, bounds);
            }
        },
        Err(_) => {
            // File prob not found so overwrite
            println!("File for bounds {:?} to {:?} written", lower_bound, higher_bound);
            return write_file(clusters, bounds);
        }
    }

    println!("File for bounds {:?} to {:?} already has better solution", lower_bound, higher_bound);
}
...
pub fn write_file(
    ...
    let (lower_bound, higher_bound) = bounds;
    image::save_buffer(
        &Path::new(&format!("examples/cluster_data/{}_{}.tiff", lower_bound, higher_bound)),
        &buffer, size[1] as u32, size[0] as u32,
        image::ColorType::L8
    ).unwrap();
    
    image::save_buffer(
        &Path::new(&format!("examples/visual_cluster_data/{}_{}.tiff", lower_bound, higher_bound)),
        &visual_buffer, size[1] as u32, size[0] as u32,
