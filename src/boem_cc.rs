...
pub fn cc_boem(
    ...
    for c in 0..cluster_indices.len() {
        update_cluster(
            clusters,
            cluster_indices,
            &mut potential_clusters,
            &mut potential_net_distance,
            c,
            correlations,
        );
    }
    ...
    for _ in 0..10000 {
        ...
        println!(
            "Move {:?} from {:?} to {:?} with improvement {:?}",
            i_star, c_original, c_star, c_star_net_distance
        );

        if c_original == c_star {
            break;
        }

        let index = cluster_indices[c_original]
            .iter()
            .position(|i| *i == i_star)
            .unwrap();
...
fn update_cluster(
    ...
    for i in &cluster_indices[cluster] {
        ...
        for c in 0..cluster_indices.len() {
            let c_net_distance =
                net_cluster_distance(cluster_indices, correlations, clusters[*i], c, *i);
...
fn net_cluster_distance(
    cluster_indices: &[Vec<(usize, usize)>],
    correlations: ArrayView4<f64>,
    c0: usize,
    c1: usize,
    i: (usize, usize),
