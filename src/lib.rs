...
use greedy_cc::{cc_vote_logarithmic, cc_vote_additive};
use boem_cc::cc_boem;
...
pub fn greedy_cc_iterations(
    ...
    // Initial run
    cc_vote_logarithmic(clusters, cluster_indices, correlations, index_permutations(indices.len()).map(|a| indices[a]), no_clusters);
    ...
    for i in 1..iterations {
        println!("Clustering...{:.2}%", (i as f32) / (iterations as f32) * 100.);
    ...
    println!("Found best cluster configuration, {:.4}% better than average", 100.*(cluster_cost_average - clusters_star_cost)/cluster_cost_average);
